...
export class FundList {
  ...
  protected readonly fundCount = computed(() => this.fundsResource.value().length);
  protected readonly totalTargetSize = computed(() =>
    this.fundsResource.value().reduce((sum, fund) => sum + fund.targetSize, 0),
  );
